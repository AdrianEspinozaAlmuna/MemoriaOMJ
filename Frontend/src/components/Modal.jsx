...
export default function Modal({
  ...
  const headerClasses = `shrink-0 flex items-center justify-between gap-4 border-b border-[#e2e7eb] bg-[#fafbfc] px-5 pb-3 pt-4 sm:pt-4 ${headerClassName}`.trim();
  ...
  const closeClasses = `h-8 w-8 cursor-pointer rounded-full border border-[#d5dce2] bg-white text-sm font-semibold text-[#385348] shadow-sm transition-colors hover:bg-[#f3f6f8] ${closeButtonClassName}`.trim();
      ...
      <div className={panelClasses}>
        {!hideHeader && (
          <header className={headerClasses}>
            <h3 className="m-0 text-[1.05rem] font-semibold tracking-tight text-[var(--text)]">{title}</h3>
